function potionEffect(pid, a, b){
	...
	if(pid == ww.potBlessRing){
		if(m != noone){
			if(m.gear[2] != noone){ itemRestore(m.gear[2]); itemEnchant(m.gear[2], 1, false); }
			if(m.gear[3] != noone){ itemRestore(m.gear[3]); itemEnchant(m.gear[3], 1, false); }
